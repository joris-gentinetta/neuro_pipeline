...
for experiment_name in experiment_names:
    ...
    mean_wave_forms = np.empty((number_of_units, number_of_channels, 60)) #1dim: units, 2dim: channels, 3dim: time from -20 to 40 frames, 20000Hz
    ...
    for unit in range(number_of_units):
        ...
        for index in range(frames.size - 1):
            bin = int((frames[index + 1] - frames[index]) * 1000 / isi_bin_size // sampling_rate) #the difference in frames converted to bins
...
sorted_units = sorted(sorted_units) #1.row: channel (corresponding to the index of the data row where the amplitude of the spike is the highest,
...
for unit in range(number_of_units):
    ...
    for index, key in enumerate(natsorted(waveform_dict.keys())):
        toplot = waveform_dict[key][unit_index,:,:] + np.linspace(0, 100*number_of_channels, number_of_channels)[:, None] #100 is the space between the channles in the plot
    ...
    for index, key in enumerate(natsorted(isi_dict.keys())):
        toplot = isi_dict[key][unit_index, :]/np.sum(isi_dict[key][unit_index, :]) #proportion of intervals in bin vs all in diagram
...
for unit in range(number_of_units):
    ...
    for index, key in enumerate(natsorted(isi_dict.keys())):
        toplot = np.where(np.sum(isi_dict[key][unit_index, :]) != 0, isi_dict[key][unit_index, :]/np.sum(isi_dict[key][unit_index, :]),0) #proportion of intervals in bin vs all in diagram
        sum+=toplot
