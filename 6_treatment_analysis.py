...
for day in days:
    ...
    archive.index = [day + '_' + str(old_index) for old_index in
                     archive_index]  # modify the index, to know from wich animal the unit is
...
for data_separation in data_separations:
    ...
    if 'phase' in toplot:
        for phase_mode in phase_modes:
            ...
            pad_columns = [column_name for column_name in column_names if
                           column_name[:len(phase_mode)] == phase_mode]  # get the columns containing the phase values

            ##compute the mean number of spikes per bin from all channles:
            mean_theta = np.zeros((data.shape[0], data.loc[:, pad_columns[0]].shape[1]),
                                  dtype=np.float32)  # rows: units, columns: phase bins
            counter = 0
            ...
            if len(animals) == 1:  # if there is only one animal, plot the phase plot for all vHIP channels individually
                treatment_plots.plot_phase_all_pads(plot_folder, data.loc[:, pad_columns], pad_columns, data_separation,
                                                    phase_mode,
                                                    show=show, save=save)
