...
def load_traj(animal, session):
    file_dir = r'E:/anxiety_ephys/' + animal + '/' + session + '\\ephys_processed\\'
...
def traj_process(animal, session, start_time, duration, behavior=None, bp='head', fps=50):
    fps = fps
    traj_df, scorer = load_traj(animal, session)
    traj_x, traj_y = calib_traj(traj_df, start_time, duration, fps, bp=bp)

    accumulated_distance, spd, acc, ismoving = calculate_speed_acc(traj_x, traj_y, start_time, duration, fps,
                                                                   move_cutoff=5)
