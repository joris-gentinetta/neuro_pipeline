...
mPFC_indices = [channel_list.index(valid_channel) for valid_channel in
                mPFC_channels]  # the indices of the valid channels of mPFC in this session
vHIP_indices = [channel_list.index(valid_channel) for valid_channel in
                vHIP_channels]  # the indices of the valid channels of vHIP in this session
amp_channels = np.array(amp_channels)
mPFC_impedance = [channel['electrode_impedance_magnitude'] for channel in
                  amp_channels[mPFC_indices]]  # mPFC impedance of the valid channels ordered by pad
vHIP_impedance = [channel['electrode_impedance_magnitude'] for channel in
                  amp_channels[vHIP_indices]]  # vHIP impedance of the valid channels ordered by pad
if select_channels:
    rhd_file = rhd_files[-2]
    rhd_data = np.array(read_data(rhd_folder + rhd_file)['amplifier_data'],
                        dtype=np.int16)  # raw ephys data of the second to last minute in the recording
    ...
    toplot = mPFC_data[:,
             mPFC_data.shape[1] // 10 * 8:mPFC_data.shape[1] // 10 * 9]  # take subset of the time to display
    sos_spike = butter(N=butter_order, Wn=spike_band, btype='bandpass', analog=False, output='sos',
                       fs=sampling_rate)  # prepare the params for the filtering
    ...
    for row, ax in tqdm(enumerate(axs)):  # plot the spike_band filtered ephys data on all valid channels ordered by pad
        label = 'channel: ' + str(mPFC_channels[row]) + ', ' + str(
            mPFC_impedance[
                row] * 100 // max_impedance) + '% max impedance'  # label with the channel number and % of max impedance
    ...
    plt.show()
    plt.close(fig)
    discarded_channels = np.array(input('discarded channels seperated by space: ').split(),
                                  dtype=np.int8)  # let the user input the names of the channels to discard
...
mouse_is_late = {'2021-02-19_mBWfus010_EZM_ephys': 70,
                 # keys: all the mice that are not present in the maze at the video trigger
...
for index, experiment_name in tqdm(enumerate(experiment_names)):
    ...
    mPFC_indices = [channel_list.index(valid_channel) for valid_channel in
                    mPFC_channels]  # the indices of the valid channels
    vHIP_indices = [channel_list.index(valid_channel) for valid_channel in vHIP_channels]
    total_size = 0
    for i, rhdfile in enumerate(rhd_files):
        rhd_data = np.array(read_data(rhd_folder + rhdfile)['amplifier_data'],
                            dtype=np.int16)  # rows are unordered channls
