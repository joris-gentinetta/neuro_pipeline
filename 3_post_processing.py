...
logbook_2[1:] = logbook_1[:len(experiment_names)] #shift to the right to make space for the 0 index of the first session
...
original_data = np.zeros((int(indexer.max()+1), original_logbook_3[-1]), dtype=bool) #rows: units, columns: 20000Hz frames
...
archive = utils.create_archive(vHIP_pads, cluster_names, number_of_bins_transitions, number_of_bins_phase, number_of_bins_isi) #create empty archive
